...
func (p *parser) parse() (*bsr.Set, []*Error) {
	...
	for !p.R.empty() {
		...
		case slot.SyntaxSuffix0R0: // SyntaxSuffix : ∙SyntaxAtom ?

			p.call(slot.SyntaxSuffix0R1, cU, p.cI)
		case slot.SyntaxSuffix0R1: // SyntaxSuffix : SyntaxAtom ∙?

			if !p.testSelect(slot.SyntaxSuffix0R1) {
				p.parseError(slot.SyntaxSuffix0R1, p.cI, first[slot.SyntaxSuffix0R1])
				break
			}

			p.bsrSet.Add(slot.SyntaxSuffix0R2, cU, p.cI, p.cI+1)
			p.cI++
			p.rtn(symbols.NT_SyntaxSuffix, cU, p.cI)
		case slot.SyntaxSuffix1R0: // SyntaxSuffix : ∙SyntaxAtom *

			p.call(slot.SyntaxSuffix1R1, cU, p.cI)
		case slot.SyntaxSuffix1R1: // SyntaxSuffix : SyntaxAtom ∙*

			if !p.testSelect(slot.SyntaxSuffix1R1) {
				p.parseError(slot.SyntaxSuffix1R1, p.cI, first[slot.SyntaxSuffix1R1])
				break
			}

			p.bsrSet.Add(slot.SyntaxSuffix1R2, cU, p.cI, p.cI+1)
			p.cI++
			p.rtn(symbols.NT_SyntaxSuffix, cU, p.cI)
		case slot.SyntaxSuffix2R0: // SyntaxSuffix : ∙SyntaxAtom +

			p.call(slot.SyntaxSuffix2R1, cU, p.cI)
		case slot.SyntaxSuffix2R1: // SyntaxSuffix : SyntaxAtom ∙+

			if !p.testSelect(slot.SyntaxSuffix2R1) {
				p.parseError(slot.SyntaxSuffix2R1, p.cI, first[slot.SyntaxSuffix2R1])
				break
			}

			p.bsrSet.Add(slot.SyntaxSuffix2R2, cU, p.cI, p.cI+1)
			p.cI++
			p.rtn(symbols.NT_SyntaxSuffix, cU, p.cI)
		...
		case slot.SyntaxSymbol1R2: // SyntaxSymbol : ! SyntaxAtom ∙
			...
			p.rtn(symbols.NT_SyntaxSymbol, cU, p.cI)
		case slot.SyntaxSymbol2R0: // SyntaxSymbol : ∙SyntaxSuffix

			p.call(slot.SyntaxSymbol2R1, cU, p.cI)
		case slot.SyntaxSymbol2R1: // SyntaxSymbol : SyntaxSuffix ∙
