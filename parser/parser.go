...
func (p *parser) parse() (*bsr.Set, []*Error) {
	...
	for !p.R.empty() {
		...
		case slot.SyntaxSuff0R0: // SyntaxSuff : ∙SyntaxAtom ?

			p.call(slot.SyntaxSuff0R1, cU, p.cI)
		case slot.SyntaxSuff0R1: // SyntaxSuff : SyntaxAtom ∙?

			if !p.testSelect(slot.SyntaxSuff0R1) {
				p.parseError(slot.SyntaxSuff0R1, p.cI, first[slot.SyntaxSuff0R1])
				break
			}

			p.bsrSet.Add(slot.SyntaxSuff0R2, cU, p.cI, p.cI+1)
			p.cI++
			p.rtn(symbols.NT_SyntaxSuff, cU, p.cI)
		...
		case slot.SyntaxSymbol1R2: // SyntaxSymbol : ! SyntaxAtom ∙
			...
			p.rtn(symbols.NT_SyntaxSymbol, cU, p.cI)
		case slot.SyntaxSymbol2R0: // SyntaxSymbol : ∙SyntaxSuff

			p.call(slot.SyntaxSymbol2R1, cU, p.cI)
		case slot.SyntaxSymbol2R1: // SyntaxSymbol : SyntaxSuff ∙
