	...
	for !p.R.empty() {
		...
		for {
			...
			case slot.Rep1F0: // Rep failure case
				p.rtn(symbols.NT_Rep, cU, failInd)
			...
			case slot.S10R0: // S1 : ∙Required Rep
				...
				p.call(slot.S10R2, slot.S11F0, symbols.NT_Rep, cU, p.cI)
			case slot.S10R2: // S1 : Required Rep ∙
			...
			case slot.SuffBase0R0: // SuffBase : ∙Base SuffBase
				...
				p.cI++
				if !p.testSelect(slot.SuffBase0R1) {
					p.parseError(slot.SuffBase0R1, p.cI, first[slot.SuffBase0R1])
					L, p.cI = slot.SuffBase1R0, cU
					goto nextSlot
				}
				p.call(slot.SuffBase0R2, slot.SuffBase1R0, symbols.NT_SuffBase, cU, p.cI)
			case slot.SuffBase0R2: // SuffBase : Base SuffBase ∙
