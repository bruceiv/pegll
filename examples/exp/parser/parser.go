	...
	for !p.R.empty() {
		...
		for {
			...
			case slot.EXP0R0: // EXP : ∙a EXP b
				...
				p.call(slot.EXP0R2, slot.EXP1R0, symbols.NT_EXP, cU, p.cI)
			...
			case slot.EXP1R0: // EXP : ∙a EXP c
				...
				if !p.testSelect(slot.EXP1R0) {
					p.parseError(slot.EXP1R0, p.cI, first[slot.EXP1R0])
					L, p.cI = slot.EXP2R0, cU
					goto nextSlot
				}
				p.bsrSet.Add(slot.EXP1R1, cU, p.cI, p.cI+1)
				p.cI++
				p.rtn(symbols.NT_EXP, cU, p.cI)
				L, p.cI = slot.EXP2M0, cU
				goto nextSlot
				...
				p.call(slot.EXP1R2, slot.EXP2R0, symbols.NT_EXP, cU, p.cI)
			case slot.EXP1R2: // EXP : a EXP ∙c
				...
				L, p.cI = slot.EXP2M0, cU
				goto nextSlot
			case slot.EXP1M0: // EXP : ∙a EXP c  [with previous match]
				...
				p.call(slot.EXP1M1, slot.EXP2M0, symbols.NT_EXP, cU, p.cI)
			case slot.EXP1M1: // EXP : a EXP ∙c
